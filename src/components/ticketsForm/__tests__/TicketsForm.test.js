...
describe('TicketsForm component', () => {
  ...
  it('renders with add Files Button', () => {
    ...
    expect(component.find('Button').findWhere((n) => n.prop('extraClass') === 'dark-full float-left')).toHaveLength(1);
  });
  ...
  it('renders TicketsForm with uploadedFiles', () => {
    const component = shallow(
      <IntlProvider
        locale="en"
      >
        <TicketsForm {...props} uploadedFiles={[{ name: 2, file: { file: 'lorem_ipsum', name: 'file name' } }]} />
      </IntlProvider>,
    ).dive().dive().shallow()
      .dive();
  ...
  it('renders TicketsForm with Feature Auctions', () => {
    ...
    expect(component.find('SelectField').findWhere((n) => n.prop('field') === 'type')).toHaveLength(1);
    expect(component.find('SelectField').findWhere((n) => n.prop('field') === 'feature_id')).toHaveLength(1);
  ...
  it('renders TicketsForm with assignedDefault', () => {
    const component = shallow(
      <IntlProvider
        locale="en"
      >
        <TicketsForm {...props} showFeaturesOptions={true} assignedDefault={{ id: 2, label: 'user name' }} />
      </IntlProvider>,
    ).dive().dive().shallow()
      .dive();
